...
class TestValidateExpectedMainSections:
    def test_no_error_when_all_main_sections_are_present(self):
        ...
        errors = validate.validate_expected_main_sections(table_template)
        assert len(errors) == 0
...
class TestValidateUnexpectedMainSections:
    def test_no_error_when_expected_main_sections_are_present(self):
        table_template = {e.value: {} for e in validate.MainSections}
        errors = validate.validate_unexpected_main_sections(table_template)
        assert len(errors) == 0

    def test_errors_for_unexpected_main_section_names(self):
        table_template = {"UNEXPECTED": "a", "NOT EXPECTED": "b"}
        errors = validate.validate_unexpected_main_sections(table_template)
        assert len(errors) == 2
